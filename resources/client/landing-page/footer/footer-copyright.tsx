...
import {FooterLogo} from '@app/landing-page/footer/footer-logo';
...
export function FooterCopyright({className, padding}: Props) {
  ...
  const divClass = clsx({
    'mt-0 justify-center items-center': isMobile,
    'mt-0': !isMobile,
  });

  const copyrightClass = clsx({
    'mt-20 flex justify-center items-center': isMobile,
  ...
  return (
    ...
    >
      ...
      <div className="mb-60 items-center justify-between gap-10 text-muted md:flex md:text-left">
...
function Menus() {
  ...
  const primaryMenu = settings.menus.find(m =>
    m.positions?.includes('footer-terms'),
  );
  const secondaryMenu = settings.menus.find(m =>
    m.positions?.includes('footer-terms-two'),
  );
