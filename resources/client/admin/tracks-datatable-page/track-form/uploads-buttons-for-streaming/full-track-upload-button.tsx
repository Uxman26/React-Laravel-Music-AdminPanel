...
import { useTrackUploaderFull } from '@app/web-player/backstage/upload-page/use-track-uploader-full';
import { useTrackUpload } from '@app/web-player/backstage/upload-page/use-track-upload';
import { useFileUploadStore } from '@common/uploads/uploader/file-upload-provider';
import { mergeTrackFormValues } from '@app/admin/tracks-datatable-page/requests/use-extract-track-file-metadata-full';

interface FullTrackUploadButtonProps {
  diskPrefix: string;
  onUploadSuccess?: (file: any) => void;
}

export function FullTrackUploadButton({ diskPrefix }: FullTrackUploadButtonProps) {
  const [uploadId, setUploadId] = useState<string>();
  const { setValue, getValues } = useFormContext<CreateTrackPayload>();
  const { openFilePicker } = useTrackUploaderFull({
    ...
    onMetadataChange: (file, newData) => {
      ...
      // Explicitly set duration_full
      if (newData.duration_full) {
        setValue('duration_full', newData.duration_full, { shouldDirty: true });
      }
    },
  });
  ...
  const handleUpload = () => {
    openFilePicker(); // Handle diskPrefix if necessary
  };
              ...
              {getValues('mp3_file') ? (
                <>
                  <span className="text-sm">
                    <Trans message={`Replace Full Track`} />
                  </span>
                  <br />
                ...
                <>
                  <span className="text-sm">
                    <Trans message={`Upload Full Track`} />
                  </span>
                  <br />
                  <span className="" style={{ fontSize: '0.6rem' }}>
                    <Trans message={`.MP3 (or .WAV)`} />
