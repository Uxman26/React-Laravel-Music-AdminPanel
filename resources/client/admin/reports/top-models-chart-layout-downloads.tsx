...
import {TopModelDatasetItem} from '@app/admin/reports/requests/use-downloads-report';
...
export function TopModelsChartLayoutDownloads({ data, isLoading, ...layoutProps }: Props) {
    ...
    >
      ...
      {dataItems.map(item => (
        item.model ? (
          ...
          <div key={item.id} className="text-muted">
            <Trans message="Model data missing." />
          </div>
        )
      ))}
      {!isLoading && !dataItems.length ? (
        <div className="flex items-center gap-8 text-muted">
          <InfoIcon size="sm" />
          <Trans message="No downloads in selected timeframe." />
        </div>
      ) : null}
    </ChartLayout>
  );
