...
import { TrackActionsBarAlbum } from '@app/web-player/tracks/track-actions-bar-album';
...
export function AlbumTrackTable({ album }: AlbumTrackTableProps) {
  const { data, sortDescriptor, onSortChange } = useSortableTableData(album.tracks);
    ...
    <div className="mt-44">
      ...
      {!album.tracks?.length && (
        <IllustratedMessage
          ...
          description={<Trans message="This album does not have any tracks yet" />}
        />
      )}
    </div>
  );
