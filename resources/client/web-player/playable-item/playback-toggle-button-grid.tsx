...
import {EqualizerImageDarkMode} from '@app/web-player/tracks/equalizer-image/equalizer-image-dark-mode';
...
interface PlaybackToggleButtonGridProps {
  queueId?: string;
...
export function PlaybackToggleButtonGrid({
  queueId,
  track,
  tracks,
  radius,
  ...
  const statusIcon = modelIsPlaying ? (
    isHover ? (
      ...
      <>
        {isDarkMode ? (
          <EqualizerImageDarkMode className={className} />
        ) : (
          <EqualizerImage color={equalizerColor} className={className} />
        )}
      </>
    )
  ) : (
