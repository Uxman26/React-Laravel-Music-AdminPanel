...
export function MediaItemStats({item, className, showPlays = true, showDownloads = true}: Props) {
  return (
    <div
      className={clsx('flex items-center gap-10 text-sm text-muted', className)}
    >
      {showPlays && <PlayCount item={item} />}
      {showDownloads && <DownloadCount item={item} />}
...
function DownloadCount({item}: DownloadCountProps) {
  ...
  return (
    <Tooltip label={<Trans message=":count downloads" values={{count}} />}>
      <div>
        <DownloadIcon size="xs" className="mr-4" />
        {count}
      </div>
    </Tooltip>
  );
