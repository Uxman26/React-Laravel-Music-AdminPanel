...
export function Waveform({track, className}: WaveformProps) {
  ...
  const [primaryColor, setPrimaryColor] = useState<string>('#fff'); // Default color

  useEffect(() => {
    // Fetch dynamic color from bootstrap data
    const { themes } = getBootstrapData();
    const color = themes.all[0]?.values['--be-primary'] || '#fff';
  ...
  useEffect(() => {
    ...
    if (
      ...
      drawWaveform(
        fakeWaveData,
        progressCanvasRef.current,
        themeValueToHex(primaryColor), // Use dynamic primary color
        5,
        3,
        track.src as string,
        analyzerData,
  ...
  }, [data?.waveData, primaryColor, track.src, analyzerData]);
