  ...
  return (
    ...
    >
      <div className="mb-24">
        ...
        />
        <div className="bg-grey-100 mx-4 mb-24 mt-24 h-px rounded"></div>
      </div>
...
function SectionTitle({ children }: SectionTitleProps) {
  ...
  return (
    <div
      className={`mx-24 mb-8 text-xs font-extrabold uppercase ${
        isDarkMode ? 'text-white' : 'text-muted'
      }`}
      style={{ color: isDarkMode ? 'svg-icon text-muted' : 'svg-icon text-night' }}
    >
      <span>{children}</span>
    </div>
  );
