...
class BuildDownloadsReport
{
    ...
    public function execute(array $params): array
    {
        ...
        $metrics = explode(',', Arr::get($params, 'metrics', 'downloads'));
    ...
    protected function createBuilder(): Builder
    {
        ...
        switch ($model) {
            case TrackDownload::class:
                // all downloads, not scoped to any resource (for admin area)
                Gate::authorize('admin.access');
                $builder = TrackDownload::query();
                break;
            case Track::class:
                $track = Track::findOrFail($modelId);
                Gate::authorize('update', $track);
                $builder = $track->downloads()->getQuery();
                break;
            case Artist::class:
                $artist = Artist::findOrFail($modelId);
                Gate::authorize('update', $artist);
                $builder = $this->artistDownloadsQuery($artist);
                break;
            case Album::class:
                $album = Album::findOrFail($modelId);
                Gate::authorize('update', $album);
                $builder = $this->albumDownloadsQuery($album);
                break;
            default:
                throw new Exception();
        }
    ...
    protected function getDownloadsMetric(): array
    {
    ...
    protected function artistDownloadsQuery(Artist $artist): Builder
    {
        return TrackDownload::query()->whereIn(
            'track_id',
            fn($q) => $q
    ...
    protected function albumDownloadsQuery(Album $album): Builder
    {
        return TrackDownload::query()->whereIn(
            'track_id',
            fn($q) => $q
