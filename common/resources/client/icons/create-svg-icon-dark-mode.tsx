...
export function createSvgIconDarkMode(
  ...
  const Component = (props: SvgIconDarkModeProps, ref: RefObject<SVGSVGElement>) => {
    ...
    const color = isDarkMode ? 'black' : 'currentColor'; // Set color based on dark mode and mobile view
...
export function createSvgIconDarkModeFromTree(
  data: IconTree[],
  displayName: string = ''
) {
  const path = treeToElement(data);
  return createSvgIconDarkMode(path!, displayName);
}
