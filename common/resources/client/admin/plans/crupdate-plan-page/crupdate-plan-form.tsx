...
export function FeatureListForm() {
  const { control } = useFormContext<CreateProductPayload>();
  const { fields, append, remove } = useFieldArray<CreateProductPayload, 'feature_list'>({
    control,
    name: 'feature_list',
    keyName: 'id',
  });
    ...
    <div>
      {fields.map((field, index) => (
        <div key={field.id} className="flex gap-10 mb-10">
          <FormTextField
            name={`feature_list.${index}.value`}  // Ensure this matches the form structure
            size="sm"
            className="flex-auto"
          />
          <IconButton
      ...
      <Button
        ...
        startIcon={<AddIcon />}
        size="xs"
        onClick={() => append({ id: new Date().toISOString(), value: '' } as FeatureListItem)}
      >
      ...
      <Button
        ...
        onClick={() => {
          append({
            ...
            interval_count: 1,
            interval: 'month',
            billing_cycle: UpsellBillingCycle.MONTHLY,
          });
        }}
      >
